/**
 * 3d/weapons/SkillWeapons.tsx - 스킬 무기 3D 통합 렌더러
 * v38 Phase 4 (S32): 6 카테고리별 통합 렌더러
 *
...
interface SkillWeaponsProps {
  projectilesRef: React.MutableRefObject<Projectile[]>;
}

export default function SkillWeapons({ projectilesRef }: SkillWeaponsProps) {
  ...
  const geometries = useMemo(() => ({
    // CODE: 큐브 (디지털 / 코드 블록)
    codeOuter: new THREE.BoxGeometry(0.2, 0.2, 0.2),
    codeCore: new THREE.BoxGeometry(0.08, 0.08, 0.08),
    // DATA: 팔면체 (데이터 결정)
    dataOuter: new THREE.OctahedronGeometry(0.14, 0),
    dataCore: new THREE.OctahedronGeometry(0.06, 0),
    // NETWORK: 이십면체 (네트워크 노드)
    networkOuter: new THREE.IcosahedronGeometry(0.14, 0),
    networkCore: new THREE.IcosahedronGeometry(0.06, 0),
    // SECURITY: 십이면체 (방어 다면체)
    securityOuter: new THREE.DodecahedronGeometry(0.14, 0),
    securityCore: new THREE.DodecahedronGeometry(0.06, 0),
    // AI: 구체 (뉴럴 노드)
    aiOuter: new THREE.SphereGeometry(0.14, 12, 12),
    aiCore: new THREE.SphereGeometry(0.06, 8, 8),
    // SYSTEM: 정사각뿔 (시스템 프로세스)
    systemOuter: new THREE.TetrahedronGeometry(0.16, 0),
    systemCore: new THREE.TetrahedronGeometry(0.07, 0),
  }), []);
